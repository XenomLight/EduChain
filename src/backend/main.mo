...
actor {
  ...
  private func generateUniqueUsername() : Text {
    loop {
      let randomNumber = nextRandom();
      let username = "User_" # Nat.toText(randomNumber);
  ...
  public shared (msg) func registerWithEmail(first_name: Text, last_name: Text, username: Text, email: Text, password: Text, confirmPassword: Text): async Result.Result<User, Error> {
    ...
    if (isEmailTaken(email) or isUsernameTaken(username)) {
...
//adnin
  public shared (msg) func addCourse(
    ...
    let admin: Principal = Principal.fromText("q6txp-dokbc-yq2x2-5w3ex-ywtty-w6dzx-vt4nf-zfay7-6pj57-d3nba-cae");  // Belum diconfigure
    if (msg.caller != admin) {
 ...
 //system
  ...
  system func postupgrade() {
    for ((p, user) in _users.vals()) {
      users.put(p, user);
    };

    for ((email, user) in _usersByEmail.vals()) {
      usersByEmail.put(email, user);
    };

    for ((username, empty) in _usernames.vals()) {
      usernames.put(username, empty);
    };
    
    for ((id, course) in _courses.vals()) {
